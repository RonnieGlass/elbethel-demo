...
const DAYS_DATA: Record<number, { title: string; subtitle: string; quote: string; sermonTitle: string; videoId: string; teaching: string; action: string }> = {
  ...
  4: {
    ...
    videoId: "SclVSQ2AasM",
    teaching: "You are the light of the world. Don't hide under the basket of insecurity. Shine so they can see Him.",
    action: "Encourage someone today."
  },
  5: {
    ...
    videoId: "vrZRhx_Lek0",
    teaching: "Rest is trust. We rest in the finished work of the Servant.",
    action: "Take 10 minutes of silence."
  },
  6: {
    ...
    videoId: "gWmR-JYKzQA",
    teaching: "Worship unifies us and invites the Commander into the room.",
    action: "Play worship music in your house."
  },
  7: {
    ...
    videoId: "H7OTdzRgSmA",
    teaching: "We celebrate because the grave is empty.",
    action: "Write down your victory."
  },
};
...
function DayModal({
  ...
  return (
    <div role="dialog" aria-modal="true" className="fixed inset-0 z-40 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="relative w-full max-w-4xl overflow-hidden rounded-3xl border border-white/10 bg-neutral-950 shadow-2xl h-[90vh] md:h-auto">
        <button
          onClick={onClose}
        ...
        <div className="grid md:grid-cols-5 h-full min-h-[500px]">
            ...
            {/* Main Content Area */}
            <div className="md:col-span-3 bg-black relative flex flex-col h-full">
                {mode === "quote" ? (
                        ...
                        <div className="p-6 md:p-8 space-y-6 overflow-y-auto flex-1">
                            ...
                            {data.teaching && (
                                <div>
                                    ...
                                    <p className="mt-2 text-neutral-300 leading-relaxed text-sm sm:text-base">{data.teaching}</p>
                                </div>
                            )}
                            {data.action && (
                                <div className="rounded-xl bg-white/5 p-4 border border-white/10">
                                    <h4 className="text-sm font-semibold uppercase tracking-wide text-amber-400">Today's Action</h4>
                                    <p className="mt-1 text-white text-sm sm:text-base">{data.action}</p>
                                </div>
                            )}
